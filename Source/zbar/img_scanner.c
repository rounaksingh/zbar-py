...
inline void zbar_image_scanner_recycle(zbar_image_scanner_t *iscn)
{
...
{
    return((iscn->syms) ? iscn->syms->head : NULL);
}
                     ...
                     unsigned width, unsigned height, const unsigned char *data)
{
    ...
    zbar_image_scanner_recycle(iscn);
    zbar_symbol_set_t *syms = iscn->syms;
    if(!syms) {
...
#ifdef ENABLE_QRCODE
    _zbar_qr_decode(iscn->qr, iscn, width, height, data);
#endif
